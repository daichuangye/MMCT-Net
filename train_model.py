...
def main_loop(batch_size=config.batch_size, model_type='', tensorboard=True):
    ...
    if config.task_name == 'MosMedData-Adjust2':
    ...
    if model_type == 'LViT':
        ...
        logger.info('transformer expand ratio: {}'.format(config_vit.expand_ratio))
        model = LViT(config_vit, n_channels=config.n_channels, n_classes=config.n_labels)
        pretrained_UNet_model_path = "Cov19/LViT/Test_session_11.07_15h45/models/best_model-LViT.pth.tar"
    ...
    logger.info('MosMedData-Adjust2数据集 Lvit-self-DownUseCovOrMax模型 LR1e-3 最小LR1e-4 BS15')
    model = model.cuda()
    ...
    if tensorboard:
        ...
        logger.info('log dir:{0}'.format(log_dir))
    ...
    for epoch in range(config.epochs):
        ...
        print('{0}轮epoch验证之后的损失和dice结果是：'.format(epoch+1),val_loss,val_dice)
